...
export default function EducationEvents() {
  ...
  useEffect(() => {
    dispatch(fetchEducationEvents({ pageNumber }));
  }, [dispatch, pageNumber]);
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteEducationEvents(id));
        setTimeout(() => {
  ...
  return (
    <>
      ...
      <TableContainer component={Paper} className="adminadvocates">
        ...
        <Table sx={{ minWidth: 650 }} aria-label="simple table">
          <TableHead>
            <TableRow>
              ...
              <TableCell align="left">Başlama vaxtı</TableCell>
          ...
          <TableBody>
            {educationEvents &&
              educationEvents.map((educationEvent) => (
                <TableRow
                  key={educationEvent.id}
                ...
                >
                  ...
                  <TableCell align="left">
                    {educationEvent?.education?.title.length > 20
                      ? educationEvent?.education?.title.slice(0, 20) + "..."
                  ...
                  <TableCell align="right" className="adminadvocates__icons">
                    <AiOutlineEdit
                      onClick={() =>
                        navigate(
                          `${ADMIN}/educationevents/${educationEvent.id}`
                        )
                      }
                      className="edit__icons"
                    />
                    <AiOutlineDelete
                      onClick={() => handleDelete(educationEvent.id)}
                      className="edit__icons"
                    />
                  </TableCell>
                </TableRow>
        ...
        <button
          onClick={() => navigate(`${ADMIN}/educationevents/create`)}
          className="adminadvocates__add"
        >
          əlavə et <AiOutlinePlusCircle className="plus__icon" />
        </button>
      </TableContainer>
    </>
  );
