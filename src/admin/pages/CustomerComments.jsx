...
export default function CustomerComments() {
  ...
  useEffect(() => {
    dispatch(fetchCustomerComment({ pageNumber }));
  }, [dispatch, pageNumber]);
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        dispatch(deleteCustomerComments(id));
        setTimeout(() => {
  ...
  return (
    <>
      ...
      <TableContainer component={Paper} className="adminadvocates">
              ...
              >
                <TableCell component="th" scope="row">
                  <img
                    ...
                    src={`${IMAGE_URL}${comment.image.filePath}`}
                    alt={comment.image.fileName}
                  />
                </TableCell>
                <TableCell align="left">{comment.fullName}</TableCell>
                <TableCell align="left">{comment.role}</TableCell>
                <TableCell align="right" className="adminadvocates__icons">
                  <AiOutlineEdit
                    onClick={() =>
                      navigate(`${ADMIN}/customercomments/${comment.id}`)
                    }
        ...
        <button
          onClick={() => navigate(`${ADMIN}/customercomments/create`)}
          className="adminadvocates__add"
        >
          rəy əlavə et <AiOutlinePlusCircle className="plus__icon" />
        </button>
      </TableContainer>
    </>
  );
